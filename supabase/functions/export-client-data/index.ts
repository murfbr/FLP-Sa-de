  ...
  try {
    ...
    if (exportType === 'session_notes') {
      ...
      const notesData = appointments
        ?.filter((a: any) => a.notes && Array.isArray(a.notes) && a.notes.length > 0)
        .flatMap((a: any) => 
          a.notes.map((n: any) => ({
            date: n.date,
      ...
      if (format === 'pdf') {
        ...
        if (notesData.length === 0) {
            ...
            notesData.forEach((note: any) => {
              ...
              doc.text(`${new Date(note.date).toLocaleDateString('pt-BR')} - ${note.service || 'Serviço'}`, 10, y)
              y += 5
        ...
        base64Content = btoa(String.fromCharCode(...new Uint8Array(pdfArrayBuffer)))
        filename += '.pdf'
      ...
      if (Array.isArray(raw)) {
        assessment = raw.find((i: any) => i.type === 'assessment' || !i.type) || {}
      ...
      if (format === 'pdf') {
        ...
        if (history.length > 0) {
          ...
          history.forEach((h: any) => {
            if (y > 270) { doc.addPage(); y = 15; }
            doc.setFont('helvetica', 'bold')
            doc.text(`Importado em: ${new Date(h.date).toLocaleDateString('pt-BR')}`, 10, y)
            y += 5
            doc.setFont('helvetica', 'normal')
    ...
    return new Response(
      JSON.stringify({ content: base64Content, filename }),
      {
