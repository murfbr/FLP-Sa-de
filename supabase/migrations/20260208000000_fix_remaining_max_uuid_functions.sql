-- Fixes "function max(uuid) does not exist" runtime errors in booking flows
-- Updates reschedule logic to respect capacity constraints
-- Ensures all UUID aggregates are properly cast to text before aggregation

-- 1. Fix book_appointment (Core Function)
...
-- 2. Fix reschedule_appointment (Core Function)
-- Adds capacity checks and fixes MAX(uuid) error
...
BEGIN
  ...
  -- FIX: Cast service_id to text before MAX()
  SELECT count(*) INTO v_conflict_count
  FROM (
    SELECT a.schedule_id
    FROM public.appointments a
...
-- 3. Update Wrapper: book_appointment_dynamic
...
BEGIN
  ...
  -- 2. Call book_appointment (which now has fix)
...
-- 4. Update Wrapper: reschedule_appointment_dynamic
...
BEGIN
  ...
  -- Call reschedule_appointment (which now has fix and capacity checks)
  PERFORM reschedule_appointment(p_appointment_id, v_new_schedule_id);
END;
$$;

-- 5. Update Wrapper: book_recurring_appointment_series
CREATE OR REPLACE FUNCTION public.book_recurring_appointment_series(
  p_professional_id UUID,
  p_client_id UUID,
  p_service_id UUID,
  p_start_time TIMESTAMPTZ,
